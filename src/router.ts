...
const CICD_COMMANDS: Record<string, CommandHandler> = {
  init:     runCicdInit,
...
export async function route(command: string, params: string[]): Promise<void> {
  if (command === 'cicd') {
    ...
    if (!sub) { await runCicdInit([]); return; }
    const handler = CICD_COMMANDS[sub];
    if (!handler) { throw new Error(`Unknown cicd subcommand: "${sub}"`); }
